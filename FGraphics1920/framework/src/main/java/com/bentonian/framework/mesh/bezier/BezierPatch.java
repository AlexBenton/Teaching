...
public class BezierPatch extends MeshPrimitive {
  ...
  private final Vec3[][] P = new Vec3[4][4];
  ...
  public Vec3 getControlPoint(int i, int j) {
    return P[i][j];
  }
  ...
  private Vec3 cubicBezier(double u, double v) {
    Vec3 P = new Vec3();
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        P = P.plus(pt(i, j, u, v));
  ...
  private Vec3 pt(int i, int j, double u, double v) {
