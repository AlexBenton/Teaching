...
public class GLCanvas {
  ...
  public void pushLineOffset() {
    projection.pushReversed(M4x4.translationMatrix(new Vec3(0, 0, -0.00001)));
  ...
  public static void updateTexture(int textureName, BufferedImage texture) {
    ...
    updateTextureBuffer(textureName, buffer, width, height, GL11.GL_RGBA, GL11.GL_LINEAR);
  }

  public static void updateTextureBuffer(int textureId, ByteBuffer buffer, int width, int height,
      int glFormat, int glTextureBlend) {
    GL11.glBindTexture(GL11.GL_TEXTURE_2D, textureId);
    GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA, width, height, 0, glFormat,
        GL11.GL_UNSIGNED_BYTE, buffer);
    GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, glTextureBlend);
    GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, glTextureBlend);
  }
  ...
  protected Vec3 getLightPosition() {
  ...
  public void updateUniformVec3(String uniformName, Vec3 v) {
    int uniform = GL20.glGetUniformLocation(program, uniformName);
    if (uniform != -1) {
  ...
  public void updateUniform3fv(String uniformName, FloatBuffer buffer) {
    int uniform = GL20.glGetUniformLocation(getProgram(), uniformName);
    if (uniform != -1) {
      GL20.glUniform3fv(uniform, buffer);
    }
  }
