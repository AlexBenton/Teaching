...
public class DemoApp extends GLFWCanvas {
  ...
  public void setCameraDistance(double d) {
    d = Math.max(0.001, d);
    Vec3 viewpoint = getCamera().getPosition();
    Vec3 delta = getCamera().getPosition().normalized().times(d).minus(viewpoint);
    getCamera().translate(delta);
  }
  
  public void animateCameraToPosition(Vec3 dest) {
    animateCameraToPosition(dest, Y_AXIS);
  }

  public void animateCameraToPosition(Vec3 dest, Vec3 up) {
  ...
  public void onKeyDown(int key) {
    ...
    case GLFW.GLFW_KEY_1:
      animateCameraToPosition(new Vec3(0, 0, sign * d));
      break;
    case GLFW.GLFW_KEY_2:
      animateCameraToPosition(new Vec3(0, sign * d, 0), new Vec3(0, 0, -sign));
      break;
    case GLFW.GLFW_KEY_3:
      animateCameraToPosition(new Vec3(sign * d, 0, 0));
      break;
  ...
  protected void onMouseDrag(int x, int y) {
    if (mouseDownCaptured) {
      ...
      Vec3 axis = getCamera().getUp().times(dx).plus(getCamera().getRight().times(dy)).normalized();
  ...
  private String getScreenshotName() {
    int i = 0;
    String screenshot = 
        FileSystemView.getFileSystemView().getHomeDirectory() + "\\" + getScreenshotTitle();
  ...
  protected void captureScreenshot(String file) {
