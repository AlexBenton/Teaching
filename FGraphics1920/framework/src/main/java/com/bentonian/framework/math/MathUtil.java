...
public class MathUtil {
  ...
  public static Vec3 intersectPlane(Vec3 linePtOne, Vec3 linePtTwo, Vec3 planePtOne, Vec3 planePtTwo, Vec3 planePtThree) {
    Vec3 N = planePtTwo.minus(planePtOne).cross(planePtThree.minus(planePtOne)).normalized();
    Vec3 D = linePtTwo.minus(linePtOne).normalized();
  ...
  public static Vec3 midPt(Vec3 a, Vec3 b) {
    return new Vec3((a.vec[0] + b.vec[0]) / 2, (a.vec[1] + b.vec[1]) / 2, (a.vec[2] + b.vec[2]) / 2);
  }
  
  public static boolean colinear(Vec3 a, Vec3 b, Vec3 c) {
    return a.minus(b).cross(c.minus(b)).length() <= EPSILON;
  }
}
