...
import com.bentonian.framework.mesh.SimpleFace;
import com.bentonian.framework.mesh.SimpleVertex;
...
public class ImplicitSurface extends MaterialPrimitive implements IsRayTraceable {
  ...
  private boolean addInterpolants(OctTreeEdge edge) {
    ...
    if (ta.force <= cutoff && tb.force > cutoff) {
      ...
      c = new SimpleVertex(a.plus(b.minus(a).times(t)), ta.color.plus(tb.color.minus(ta.color).times(t)));
      edge.setCrossingData(c, a.minus(b).normalized());
      return true;
    } else if (tb.force <= cutoff && ta.force > cutoff) {
      t = (cutoff-tb.force) / (ta.force-tb.force);
      c = new SimpleVertex(b.plus(a.minus(b).times(t)), tb.color.plus(ta.color.minus(tb.color).times(t)));
  ...
  @Override
  public synchronized void renderLocal(GLRenderingContext canvas) {
    ...
    if (!surfaceVao.isCompiled()) {
      ...
      for (Octree octree : getFinished()) {
        for (SimpleFace poly : octree) {
          ...
          if (blendColors) {
            surfaceVao.color(poly.a.color);
            surfaceVao.vertex(poly.a);
            surfaceVao.color(poly.b.color);
            surfaceVao.vertex(poly.b);
            surfaceVao.color(poly.c.color);
  ...
  public Mesh getMesh() {
    Mesh mesh = new Mesh();
    Map<SimpleVertex, Vertex> vertices = Maps.newHashMap();
    for (Octree octree : getFinished()) {
      for (SimpleFace poly : octree) {
      ...
      if (octree.isEmpty()) {
        ...
        for (SimpleFace poly : octree) {
          ...
          if (t != null) {
            ...
            hits.add(t, P, approximateNormal, material);
          }
        }
      }
    }
