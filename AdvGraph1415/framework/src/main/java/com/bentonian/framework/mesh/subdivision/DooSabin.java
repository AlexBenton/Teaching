...
public class DooSabin implements SubdivisionFunction {
  ...
  public Mesh apply(Mesh mesh) {
    ...
    for (MeshFace face : mesh) {
      for (MeshVertex v : face) {
        MeshEdge e = new MeshEdge(v, face.getVertex(v.getFaceIndex(face) + 1));
        if (e.isBoundaryEdge()) {
    ...
    for (MeshFace face : mesh) {
      ...
      for (int i = 0; i < face.size(); i++) {
        MeshVertex A = face.getVertex(i + 1);
        MeshVertex B = face.getVertex(i);
        MeshVertex C, D;
    ...
    for (MeshVertex v : mesh.getVertices()) {
      ...
      if (boundary != null) {
        ...
        MeshEdge e = new MeshEdge(v, first.getVertex(v.getFaceIndex(first) + 1));
      ...
      if (boundary != null) {
        ...
        MeshEdge e = new MeshEdge(v, last.getVertex(v.getFaceIndex(last) - 1));
        Map<MeshVertex, MeshVertex> edgeNextGen = boundaryNextGen.get(e);
        vertexNextGen[j++] = edgeNextGen.get(v);
      }
