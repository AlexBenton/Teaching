...
public class SubdivisionDemo extends DemoApp {
  ...
  private final Map<Coord, List<MeshPrimitive>> levels = Maps.newHashMap();
  private final Map<Coord, List<AnimatedMeshPrimitive>> animations = Maps.newHashMap();
  ...
  protected SubdivisionDemo() {
    ...
    for (int fn = 0; fn < NUM_FUNCTIONS; fn++) {
      for (int geom = 0; geom < GEOMETRY.length; geom++) {
        ...
        animations.put(coord, Lists.<AnimatedMeshPrimitive>newArrayList());
        levels.put(coord, Lists.<MeshPrimitive>newArrayList());
